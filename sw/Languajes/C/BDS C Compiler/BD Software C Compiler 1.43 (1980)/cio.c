 ...
 *
 * NOTE: This set of functions is designed as an alternative to the normal
 * 	 getchar, putchar, etc. residing in deff2.crl; If you don't really
 *	 need the TTYMode features of this interface, the normal versions
 *	 of the functions are much more efficient. If you DO want to use
 *	 this set of functions, then compile this file once (to obtain CIO.CRL)
 *	 and link your programs in the following manner:
 *
 *		A>clink main <other primary CRL files, if any> DEFF CIO <cr>
 *
 *	 where "main" is the CRL file containing the main function for your
 *	 programs. 
 */

#include "bdscio.h"			/* BDSCIO.H console "#defines" must */
...
TTYMode(mode)
{
	return rawio(4, mode); 		/* set mode bits, returns prev. */
}
