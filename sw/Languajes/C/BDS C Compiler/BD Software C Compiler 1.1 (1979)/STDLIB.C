...
char *format;
{
	...
	args = (RAM + 0x3f9);
	/* warning: none of the functions called here can have more than one parameter
	   to not mess args values*/
	...
	while (x = *format++) 
	   switch(x) {
		...
		switch(y) {
		   case 'd': 
		  	if(*args<0) { putchar('-'); *args= -*args; width--; }
