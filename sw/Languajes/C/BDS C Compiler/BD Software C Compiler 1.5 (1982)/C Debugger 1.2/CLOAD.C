...
process to a BDS C relocatable object image.  To use the program, you do the
following:
...
The upshot of all of this is that the CASM process changes as follows:

	   old way			   new way

	A>casm foo			A>casm foo
	A>asm foo.aaz			A>asm foo.aaz
	A>ddt foo.hex			A>cload foo
	-g0
	A>save nn foo.crl

Now ain't that simpler?
*/
...
char *argv[];
{
    ...
    puts("CASM Image Hex-to-Crl Converter -- Version 0.0\n");
    ...
    if (fopen(hexname,hex) == ERROR) die(NULL);
    if ((crl = creat(crlname)) == ERROR) die(NULL);

    bbase = bsize = endext();  bend = topofmem() - STACKSZ;
    ...
    i = (bsize - bbase + (SECSIZ - 1)) / SECSIZ;
    die((write(crl,bbase,i) != i || fclose(hex) == ERROR ||
	close(crl) == ERROR) ? NULL : "CRL file successfully generated");
}
