/*
	...
	modified for BDS C by Leor Zolman
        modified for Hisoft C by Dave Howorth
         - correct some function type errors
         - remove an & from call to qsort (its not right on an array)
         - change to unix-style rand() function
         - make variables static to save hundreds of bytes
...
commands may be typed:
	...
	g	causes computer to play both sides until game
		is over or until you hit a key.
...
#data 0x4000	/* end of initialisers + length of globals */
#error
...
int n;
{
  ...
  static int i,j;
  static int handicap;
  static int h[4][2] = {0, 0,	/* handicap position table */
                        7, 7,
                        0, 7,
                        7, 0};
...
{
  static char a[8][8];
  static int ok,s,k,l,side,oside;
  static int c,dkl;
	...
	putmov(a,p,i,j);
        ...
        if (i==1 || i==6) side++;  /*DJH*/
        if (j==1 || j==6) side++;  /*DJH*/
        if (side == 2) return -19; /*DJH*/
	side = 0;
        if (i==0 || i==7) side++;
        if (j==0 || j==7) side++;
        if (side == 2) return 5;   /*DJH*/
	s = 0;
	ok = 0;
