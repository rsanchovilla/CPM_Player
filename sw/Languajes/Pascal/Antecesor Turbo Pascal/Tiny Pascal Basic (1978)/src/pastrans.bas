...
16 REM ********************************************************
17 REM
20 ! \ ! \ !"INITIALIZING 8080 TRANSLATOR"
...
310 INPUT" HEX ADDRESS OF 8080 RUNTIME CODE (<CR>=1A00): ",B0$
311 IF B0$=""THEN B0$="1A00"
312 X=FND(B0$,4)-FND("1A00",4)
313 INPUT" HEX ADDRESS OF TINY PASCAL PCODE (<CR>=C000): ",A$
315 IF A$=""THENA$="C000"
320 X=FND(A$,4)\X0=X
330 INPUT" HEX ADDRESS OF 8080 PROGRAM CODE (<CR>=B000): ",B$
335 IF B$=""THEN B$="B000"\P0=FND(B$,4)
340 INPUT" STACK START ADDR.(<CR>=A000):",B$
345 IF B$<>""THEN 360
350 !" DEFAULT END STACK ADDRESSES USED (AFFF)"
355 A$="AFFF"\GOTO365
360 INPUT" STACK END ADDR:",A$
365 K=65536-FND(A$,4)
...
2000 !"GENERATE PROG.BIN FILE (8080 CODE)"
2010 DESTROY "PROG.BIN"\CREATE "PROG.BIN"
2020 OPEN#1,"PROG.BIN"
2030 FOR I=P0 TO P1\C=EXAM(I)\WRITE#1,&C,NOENDMARK\NEXT
2035 E=195\D=0\C=0\WRITE#1,&E,&D,&C\REM End of prg mark
2040 CLOSE#1
2045 !"LOADING PASLOADER"
2050 REM EXEC: PIP PROG.COM=PASRUN1A+PROG.BIN
2060 FILL 129,2\CHAIN "PASLOAD"
3000 OUT\END
