h55692
s 00000/00068/00791
d D 1.2 83/03/21 18:20:47 mrk 2 1
c removed fortran setbit, put it into setbit.prtx
e
s 00859/00000/00000
d D 1.1 83/03/17 15:28:51 tes 1 0
c date and time created 83/03/17 15:28:51 by tes
e
u
4
U
...
T
I 1
subroutine ddprtx (contrl, intin, ptsin, intout, ptsout)
########################################################################
#                                                                      #
...
#          ALL RIGHTS RESERVED.                                        #
#                                                                      #
#     Function: Device Driver for the Printronix P300 and P600 printers#
#                                                                      #
#     Input Parameters:                                                #
...
#             outint - Output two byte integers to the object list     #
#             outxy  - Output two 16 bit integers to the object list   #
...
define(`RASTERxBUFFER',80)
define(`PRINTxSIZE',1)
define(`NUMBERxOFxROWS',10)
define(`RASTERxLENGTH',eval(10*80))
define(`SCANxWIDTH',eval(10*1))
...
data iout1/XxLIMITSxPRINTER,   # Maximum addressable width of printer
	   ...
	   424,                # Width of one pixel in micrometers
	   353,                # Height of one pixel in micrometers
	   ...
	   0,                  # Number of available colors, monochrome
	   0,                  # Number of available locator devices
	   0,                  # Number of available valuator devices
	   0,                  # Number of available choice devices
	   0,                  # Number of available string devices
...
CLOSExWORKSTATION  continue
   ...
   call makras
   call prtout (LF)  # Extra line feed to terminate things correctly
   go to ENDCASE
...
#                                                                      #
#     Routines Called:                                                 #
#            getxy  - get 2 16 bit integers from the object list       #
...
for (; ytop >= 0;) {  # Keep going until top is off the page
   ...
   do i = 1,RASTERxLENGTH {rasbuf(i) = ATSIGN }# Initialize buffer for each scan
   ...
   go to (OPENxWORKSTATION, CLOSExWORKSTATION, CLEARxWORKSTATION,
	 ...
	 if (iatt == BAR)   {
	    ...
	    call getxy (xy2)
            ...
            ibuf(1) = xy1(1)  # put the corners into polygon form
            ibuf(2) = xy1(2)
            ibuf(3) = xy2(1)
            ibuf(4) = xy1(2)
            ibuf(5) = xy2(1)
...
# Include the printer primitives,: line, text, and filled areas
include(`ddprim.r')
D 2
...
#            mult   - multiply two 16 bit integers                     #
########################################################################
integer i, j, m, index, base, end, mult
...
common /raster/ ytop, ybot, rasbuf
   ...
   do m = 1, NUMBERxOFxROWS   {
      ...
      for (i=end; i>base; i=i-1) {
         if (rasbuf(i) != ATSIGN)  break
         }
   
      call prtout (ENQ)  # Enter graphics mode
      ...
      call prtout (LF)   # Advance a graphics line

      base = end   # And the end becomes the new base
      }
   return
end
# Include data handling and file spooling
include(`data.r')
E 1
