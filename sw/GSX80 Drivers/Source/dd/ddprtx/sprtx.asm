h07223
s 00179/00000/00000
d D 1.1 83/03/17 15:29:09 tes 1 0
c date and time created 83/03/17 15:29:09 by tes
e
u
4
U
...
;
	 ...
	 lhld     ytop
	 call     isub
	 xchg              ; Put the row in DE
	 lxi      h,  0    ; HL hold the index into RASBUF
	 lxi      b,  RASTERBUFFER  ; increment of the raster buffer

findrow: dcx      D        ; decrement the row count
	 mov      a, d     ; determine if the count went negative
	 ora      d
	 jm       foundit
	 dad      b        ; Increment the index for a new row
	 jmp      findrow

foundit: push     h        ; Save the index for now on the stack

	 ; divide the xcoord by 6 to get the rest of the index
	 ; also provides the bit to set too
         lxi      b, remain
	 lxi      d, const6
	 lxi      h, xcoord  
	 call     divid    ; use the standard division module
         
	 pop      d        ; Get the other part of the index from the stack
	 dad      d
	 push     h        ; Save the index on the stack

	 lhld     remain   ; Get the mask into HL
	 call     power2   ; Rotate the row into a bit
	 xchg              ; save the mask in DE

	 pop      h        ; Get the index from the stack
	 lxi      b,  rasbuf
