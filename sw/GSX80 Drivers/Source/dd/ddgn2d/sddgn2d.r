h33351
s 01750/00000/00000
d D 1.1 83/03/15 21:18:53 tes 1 0
c date and time created 83/03/15 21:18:53 by tes
e
u
4
U
...
T
I 1
subroutine ddgn2d (contrl, intin, ptsin, intout, ptsout)
########################################################################
#                                                                      #
...
#          ALL RIGHTS RESERVED.                                        #
#                                                                      #
#     Function: Device driver for Datamedia Colorscan 10 terminal with #
#               Digital Engineering GEN.II Retro-Graphics              #
#                                                                      #
#     Input Parameters:                                                #
...
#             drgen2 - move/draw for GEN II                            #
#             ttyin  - get a character from the device                 #
#             gn2out - put a character to device                       #
#             strout - put a character string to the device            #
#             strinp - get a string of characters from the device      #
...
integer rdgen2(7), grgen2(7), blgen2(7)
...
integer i, j, k, l, line(30), nx, ny, circle(5), pie(9),
	...
	lstyle(7), kstyle, nchar, vector(4), iniary(45), chrhgt,
        temp, kstop, gitoch, cscale(10), crot(9), raster(12), celwdt
integer xbar(4), ybar(4), poly(4), gimnmx, xtemp, ytemp,
	...
	yrot(4), xoffst(4), yoffst(4), curup(3), curdwn(3), currgt(3),
	curlft(3), curhom(3), erscrn(3), erslin(3), revon(4), revoff(4)
ifdef(`F80',`
integer inqcur(4)
',`
integer inqcur(5)
...
integer xrpt, yrpt, m, n, pixel, xincr, yincr, savex, savey, colptr, xcoord,
	ycoord, xcount, ycount, rdback(7)
')
 
include(`ddcom')
 
common /cmgen2/ rdgen2, grgen2, blgen2       # gen2 dd common
...
ifdef(`F80',`
data inqcur /ESC, LBRACK, DIG6, LETN/
',`
data inqcur /ESC, LBRACK, DIG6, LETN, NEWLINE/
')

#            Device dependent information array
data iniary/XxLIMITSxGEN2,   # Addressable width in rasters of screen
            YxLIMITSxGEN2,   # Addressable height in rasters of screen
            ...
            1,          # Number of fonts
            120,        # Number of patterns
            0,          # Number of hatch styles
            7,          # Number of predefined colors
            4,          # Number of GDPs
            1,          # Bar GDP
            2,          # Arc GDP
            3,          # Pie Slice GDP
            4,          # Circle GDP
            -1,-1,-1,-1,-1,-1,   # Items 20 - 25 to fill out the GDP list
...
data ptsini/ 0,
             ...
             0,
             0,
             10,          # Minimum marker height in device units
             0,
             10/          # Maximum marker height in device units
...
if (opcode == OPENxWORKSTATION) {
   ...
   ndclrl = gimnmx (intin(3), 0, 6) + DIG0  # Current polyline color

   ndmktp = intin(4)      # Current polymarker type
   if (ndmktp < 1 | ndmktp > 5) ndmktp = 3  # use default

   ndclrm = gimnmx (intin(5), 0, 6) + DIG0  # Current polymarker color
   ndclrt = gimnmx (intin(7), 0, 6) + DIG0  # Current text color
   ...
   ndclrf = gimnmx (intin(10), 0, 6) + DIG0 # Current fill area color
   ...
   # Mix colors
   do i=1,7 {
      call dmpgn2 (i-1, rdgen2(i), grgen2(i), blgen2(i))
      }

   call dcgen2 (DIG1)         # Color index one is default line color
   ...
   crot(6) = DIG0             # Use zero rotation as default
   crot(7) = CR
   ifdef(`F80',`
   call strout (7, crot)      # Output text rotation command
   ',`
   call gdstot (7, crot)
   ')
   ...
   txtatt(6) = BIGC           # Use text color C1
   txtatt(7) = DIG1
   ifdef(`F80',`
   call strout (8, txtatt)    # Output text attribute command
   ',`
...
#
   ...
   call dmpgn2 (0, 100, 100, 100)   # Map foreground text color to white
   ifdef(`F80',`
...
#
	...
	ifdef(`F80',`
	call strout (2, curhom)   # Position cursor command
	',`
	call gdstot (2, curhom)
	')
	...
	ifdef(`F80',`
	call strout (4, inqcur)                # Transmit inquiry command
	i = 0
	call ttyin (i)                         # Skip the leading ESC and [
	call ttyin (i)
	',`
	...
	')
	i = 0
	repeat {                               # Read until terminator R found
	   ifdef(`F80',`
	   j = 0
	   call ttyin (j)
	   ',`
...
#
   ...
   call dcgen2 (ndclrm)        # Change color if necessary
 
   j = 1
   k = contrl(2)
   do i=1,k {                  # Loop through all points
      call dmgen2 (ptsin(j))   # Emulate a marker
      j = j + 2
      }
...
#
   ...
   # adjust the cursor position both vertically and horizontally.  The 
   # directions of these adjustments are based on the current text
   # rotation.
   ...
   do i=1,temp {               # Search for delimiter
      if (intin(i) == TILDE) intin(i) = BLANK  # Replace with blank
      }
...
#
   ...
   call dfgen2                          # Change fill color if necessary
...
ifdef(`COLORxBOX',`
   # Override current writing mode
   i = BIGN                      # Assume replace mode
   if (contrl(9) == XORxMODE) i = BIGY   # X-or mode
   wrtmod(6) = i                 # Select write mode
   ifdef(`F80',`
   ...
   do m=1,ny {                          # Traverse this many rows
	 ...
	 do n=1,nx {                    # Use this many elements per row
            ...
            pixel = gimnmx (intin(colptr), 0, 6)  # Bound the element
   ...
   # Reset writing mode
   i = BIGN                      # Assume replace mode
   if (ndotkt == XORxMODE) i = BIGY   # X-or mode
   wrtmod(6) = i                 # Select write mode
   ifdef(`F80',`
   call strout (7, wrtmod)       # Set writing mode
   ',`
...
#
   ...
   if (opcode == BAR) {
      ...
      xbar(1) = ptsin(1)         # Compute vertices of polygon
      ybar(1) = ptsin(2)
      xbar(2) = ptsin(3)
      ybar(2) = ybar(1)
      xbar(3) = xbar(2)
      ...
      ifdef(`F80',`
      call strout (4, poly)               # Output polygon (bar)
      ',`
      call gdstot (4, poly)
      ')
      do i=1,4 {
	 call xygen2 (xbar(i), ybar(i))  # Output the bar
         }
      ifdef(`F80',`
      ...
      ifdef(`F80',`
      call strout (ilen, line)   # Output command
      ',`
...
#
   ...
   ndtxsz = divid (mult (chrhgt, 100)+10, 21, j)   # Compute scale factor
   temp = divid (mult (ndtxsz, 13)+50, 100, j)     # 1/2 cell width
   celwdt = mult (temp, 2)        # Full cell width.  Will be multiple of 2

   # Only scale factors which produce cell widths that are multiples of 4
   # are used to get around the problem of discrepancies in character size
   # between horizontal and vertical text of the same size scale factor.
   # This restriction allows the use of the same scale factor for rotated
   # text as well as horizontal text, since scale factors that produce cell
   # widths that are multiples of 4 produce the same cell width for both
   # rotated and non-rotated text.  The following calculations are needed
   # to determine the correct character scale factor if the cell width
   # is not a multiple of 4.

   if (mod (celwdt, 4) != 0) {
      temp = divid (celwdt, 2, j)                 # Intermediate cell width
      ...
      celwdt = celwdt - 2       # New cell width.  Now is a multiple of 4
      }

   ilen = gitoch (ndtxsz, cscale(6), 4, ierror)  # Convert scale to characters
   ilen = ilen + 6              # Position command terminator
   cscale(ilen) = CR
   ifdef(`F80',`
   ...
   ptsout(3) = celwdt                            # Return char cell width
   temp = divid (mult (14, ndtxsz)+50, 100, j)   # Calculate 1/2 cell height
   ptsout(4) = mult (temp, 2)                    # Return char cell height
   }
...
#
   ...
   # Allow character rotation in 90 degree multiples
   # Device can actually do 1 degree multiples, but
   # mathematics to do so is infeasible in an integer
   # environment.

   ndtxrt = mult (divid ((intin(1)+450), 900, i), 900)  # Round up angle
   i = divid (ndtxrt, 3600, ndtxrt)
   ilen = gitoch (divid (ndtxrt, 10, i), crot(6), 3, ierror)
   ilen = ilen + 6
   crot(ilen) = CR
   ifdef(`F80',`
...
#
   ...
   i = gimnmx (intin(1), 0, 6)         # Seven possible color indices
   k = i + 1
   rdgen2(k) = divid (intin(2), 10, temp)   # Save RGB values
   grgen2(k) = divid (intin(3), 10, temp)
   blgen2(k) = divid (intin(4), 10, temp)
   call dmpgn2 (i, rdgen2(k), grgen2(k), blgen2(k))
   }
...
#
   ...
   ndlntp = ndlntp + DIG0           # Set linestyle
   kstyle = ndlntp
   ifdef(`F80',`
   call strout (7, lstyle)
   ',`
...
#
   ...
   ndclrl = gimnmx (intin(1), 0, 6)         # Seven line color indices
   intout(1) = ndclrl
   ndclrl = ndclrl + DIG0
   }
...
#
   ...
   ndclrm = gimnmx (intin(1), 0, 6)        # Seven marker color indices
   intout(1) = ndclrm
   ndclrm = ndclrm + DIG0
   }
...
#
   ...
   ndclrt = gimnmx (intin(1), 0, 6)        # Seven text color indices
   intout(1) = ndclrt
   ndclrt = ndclrt + DIG0
   }
...
#
   ...
   if (ndflpt < 1 | ndflpt > 6) ndflpt = 1
   intout(1) = ndflpt           # Return pattern fill index
   ndflpt = ndflpt + SLASH      # Convert pattern fill index to character
   }
...
#
   ...
   ndclrf = gimnmx (intin(1), 0, 6)         # Seven fill color indices
   intout(1) = ndclrf
   ndclrf = ndclrf + DIG0
   }
...
#
   ...
   i = gimnmx (intin(1), 0, 6) + 1    # Seven possible color indices
   intout(1) = i - 1                  # Return index selected
   intout(2) = mult (rdgen2(i), 10)   # Return color representation values
   intout(3) = mult (grgen2(i), 10)
   intout(4) = mult (blgen2(i), 10)
   }
...
ifdef(`COLORxBOX',`
   ...
   do ycount=1,i {
	 ...
	 if (xcoord <= XxLIMITSxGEN2 & ycoord <= YxLIMITSxGEN2) {
	    ...
	    if (line(1) >= BIGA) k = line(1) - ATSIGN    # Pixel is on
...
#
   ...
   # Map color index 0 to white for the crosshair cursor
   call dmpgn2 (0, 100, 100, 100)
...
#
   ...
   ndotkt = intin(1)
   if (ndotkt == XORxMODE) j = BIGY   # Complement mode
   else {
...
subroutine dmpgn2 (index, red, green, blue)
########################################################################
#                                                                      #
#          THIS MATERIAL IS CONFIDENTIAL AND IS FURNISHED UNDER        #
#          A WRITTEN LICENSE AGREEMENT.  IT MAY NOT BE USED,           #
...
#                                                                      #
########################################################################
integer index, red, green, blue, gitoch, setclr(20), clr(3)
...
data setclr/ COM_CHAR,BIGM,BIGI,BIGX,BLANK,BIGC,0,BLANK,0,0,0,0,0,0,0,_
   ...
   do i=1,3 {
      ilen = ilen + 1
      j = gitoch (clr(i), setclr(ilen), 3, ierror)
      ilen = ilen + j
      setclr(ilen) = COMMA
      }
   setclr (ilen) = CR
   ifdef(`F80',`
   call strout (ilen, setclr)
   ',`
...
data patdat/ 14*129,_                                       # Vertical (P1)
   ...
   if (ndfstl == HOLLOW | ndfstl == SOLID) { # Handle hollow and solid
      filclr(6) = BIGC
   ...
   if (ndfstl == PATTERN | ndfstl == HATCH) {  # Pattern and hatch are the same
	 ...
	 l = ilen + 1
      ...
      filclr(6) = BIGP          # Set pattern number
      filclr(7) = ndflpt
      filclr(9) = BIGC
      filclr(10) = ndclrf
      ifdef(`F80',`
...
data fcolor/ COM_CHAR,BIGC,BIGO,BIGL,BLANK,BIGC,BLANK,CR/

   if (ndclrp != color) {       # Does color need to be changed
      ndclrp = color            # Set the current color
      fcolor(7) = color         # Store color in command
      ifdef(`F80',`
      call strout (8, fcolor)   # Output the command
      ',`
...
#          ALL RIGHTS RESERVED.                                        #
#                                                                      #
#     Function: Output coordinate pairs to the GEN.II device           #
#                                                                      #
#     Input Parameters:                                                #
...
#                                                                      #
########################################################################
integer ilen, kx, ky, line(4), gitoch, ierror
 
include(`ddcom')
 
   # Output string will look as follows: BLANKxxx,yyy
   line(1) = BLANK
   ilen = gitoch (kx, line(2), 3, ierror)
   ifdef(`F80',`
   call strout (ilen+1, line)
   ',`
   call gdstot (ilen+1, line)
   ')
   line(1) = COMMA
   ilen = gitoch (ky, line(2), 3, ierror)
   ifdef(`F80',`
   call strout (ilen+1, line)
   ',`
   call gdstot (ilen+1, line)
