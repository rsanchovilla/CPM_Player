h40687
s 01724/00000/00000
d D 1.1 83/03/15 21:17:07 tes 1 0
c date and time created 83/03/15 21:17:07 by tes
e
u
4
U
...
T
I 1
subroutine ddgn2b (contrl, intin, ptsin, intout, ptsout)
########################################################################
#                                                                      #
...
#          ALL RIGHTS RESERVED.                                        #
#                                                                      #
#     Function: Device driver for ADDS Viewpoint GEN.II Retro-Graphics #
#               terminal                                               #
#                                                                      #
#     Input Parameters:                                                #
...
#             muldiv - perform integer division and multiplication     #
#                                                                      #
########################################################################
...
data erascr/ ESC, LETK/

#            Erase to end of line
data eralin/ ESC, BIGK/

#            Load cursor address
data craddr/ ESC, BIGY, 0, 0/
...
#
   ...
   if (opcode == INQUIRExADDRESSABLExCELLS) {
      ...
      # Start position must be known
      row = 24
      column = 1
      }
   else if (opcode == CURSORxUP) {

      if (row > 1) {   # Take action only when not at top of display
         ifdef(`F80',`
         call gn2out (SUB)
         ',`
         call gdevot (SUB)
         ')
         row = gimnmx ((row - 1), 1, 24)
	 }
      }
   else if (opcode == CURSORxDOWN) {

      if (row < 24) {   # Take action only when not at bottom of display
         ifdef(`F80',`
         call gn2out (LF)
         ',`
         call gdevot (LF)
      ...
      if (column < 80) {   # Take action only if not at right edge of display
         ...
         call gdevot (ACK)
         ')
         column = gimnmx ((column + 1), 1, 80)
	 }
      }
   else if (opcode == CURSORxLEFT) {

      if (column > 1) {   # Take action only when not at left edge of display
         ifdef(`F80',`
         call gn2out (NAK)
         ',`
         call gdevot (NAK)
   ...
   else if (opcode == DIRECTxCURSORxADDRESS | opcode == HOMExCURSOR) {
      # The home command for this device puts the cursor in the lower
      # left hand corner of the display, so the direct cursor address
      # command must be utilized to put the cursor in the upper left
      # corner of the display
      if (opcode == DIRECTxCURSORxADDRESS) {
   ...
   else if (opcode == PLACExCURSORxATxLOCATION) {
      ifdef(`F80',`
      call gn2out (CAN)
      ',`
      call gdevot (CAN)
...
#
   ...
   ndclrp = -1                  # Make sure driver will set correct color
   txtatt(6) = BIGC
   txtatt(7) = ndclrt
   ifdef(`F80',`
...
#
   ...
   if (opcode == BAR) {
	 ...
	 call xygen2 (xbar(i), ybar(i))     # Output the coordinates
      ...
      ifdef(`F80',`
      call strout (4, vector)         # Move to start position
      ',`
      call gdstot (4, vector)
      ')

      call xygen2 (ptsin(1), ptsin(2))   # Output start coordinates
      ifdef(`F80',`
...
include(`ddcom')
   ...
   i = ilen + 2
   line(i) = COMMA
   ilen = gitoch (ky, line(i+1), 3, ierror)
   ifdef(`F80',`
   call strout (ilen+i, line)
   ',`
