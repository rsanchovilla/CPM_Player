h03771
s 01768/00000/00000
d D 1.1 83/03/15 21:17:58 tes 1 0
c date and time created 83/03/15 21:17:58 by tes
e
u
4
U
...
T
I 1
subroutine ddgn2c (contrl, intin, ptsin, intout, ptsout)
########################################################################
#                                                                      #
...
#          ALL RIGHTS RESERVED.                                        #
#                                                                      #
#     Function: Device driver for Televideo 910 GEN.II Retro-Graphics  #
#               terminal                                               #
#                                                                      #
#     Input Parameters:                                                #
...
#            Read cursor address
data rdcrsr/ ESC, QMARK/

#            Set cursor visibility
data curvis/ ESC, PERIOD/
...
#
   ...
   if (opcode == INQUIRExADDRESSABLExCELLS) {
      ...
      # Start position should be a known location
      row = 24
      column =1
      }
      ...
      if (row > 1) {   # Take action only if not at top of display
         ...
         call gdevot (VT)
         ')
	 row = row - 1   # Save row to prevent scrolling
	 }
      }
	 ...
	 row = row + 1   # Save row to prevent scrolling
      ...
      if (column < 80) {   # Take action only if not at right edge of display
         ...
         call gdevot (FF)
         ')
	 column = column + 1  # Save column to prevent wrap-around
	 }
      }
	 ...
	 column = column - 1  # Save column to prevent wrap-around
      ...
      call gdevot (RS)
      ')
      row = 1      # Save position to prevent scrolling and wrap-around
      column = 1
      }
      ...
      row = gimnmx (intin(1), 1, 24)     # Save position in common so scrolling
      column = gimnmx (intin(2), 1, 80)  # and wrap-around will be prevnted
      craddr(3) = row + US
      craddr(4) = column + US
      ifdef(`F80',`
      ...
      ifdef(`F80',`
      call strout (2, rdcrsr)
      call strinp (NOxECHO, 3, line, temp)
      ',`
      call gdstot (2, rdcrsr)
      call gdstin (3, line, temp)
      ')
      row = line(1) - US     # Save position so scrolling and wrap-around 
      column = line(2) - US  #  will be prevented
      intout(1) = row
      intout(2) = column
      }
   else if (opcode == PLACExCURSORxATxLOCATION | opcode == REMOVExCURSOR) {
      ifdef(`F80',`
      call strout (2, curvis)
      ',`
      call gdstot (2, curvis)
      ')
      }
   }
