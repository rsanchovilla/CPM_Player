h33124
s 00840/00000/00000
d D 1.1 83/03/15 21:10:39 tes 1 0
c date and time created 83/03/15 21:10:39 by tes
e
u
4
U
...
T
I 1
subroutine dd7220 (contrl, intin, ptsin, intout, ptsout)
########################################################################
#                                                                      #
...
#          ALL RIGHTS RESERVED.                                        #
#                                                                      #
#     Function: Device Driver for Hewlett Packard 7220 pen plotter     #
#                                                                      #
#     Input Parameters:                                                #
...
#             Dr7220 - move/draw for HP plotter                        #
#             Dp7220 - Change pens on the HP 7220 plotter              #
#             Dt7220 - Set text size on HP 7220 plotter                #
...
divert(-1)
# local defines
define(`XxLIMITSxHP7220',15200)            #plotter surface width
define(`YxLIMITSxHP7220',10000)            #plotter surface height
define(`MAXxHPxBUFFER',80)
define(`NUMBERxOFxPENS',8)                 # 8 pens
define(`MOVE',1)
define(`DRAW',2)
define(`YES',1)
define(`NO',0)
...
common /cm7220/ clrred, clrgrn, clrblu
...
data iniino/_			# initial intout array on open workstation
   XxLIMITSxHP7220,# Maximum addressable screen width
   YxLIMITSxHP7220,# Maximum addressable screen height
   METERS,         # Device coordinates are meters
   25,             # X step size in micrometers
   25,             # Y step size is micrometers
...
data inipto/_	   # initial ptsout array in open workstation
   0,
   60,             # Minimum character height in DC space
			       # .15cm = 60 steps 
   0,
   eval(YxLIMITSxHP7220/2),    # Maximum character height in DC space
			       # 18 cm = 7200 steps 
   ...
   80,             # Minimum marker height in DC space
			       # .20cm = 80 steps 
   0,
   eval(YxLIMITSxHP7220/2)/    # Maximum marker height in DC space
			       # 18 cm = 7200 steps 
...
if (opcode == OPENxWORKSTATION) {
   ...
   do i=1, 7  {  # Start out by all being black
      clrred(1) = 0
      clrgrn(1) = 0
      clrblu(1) = 0
      }
   ...
   call dt7220 (ndtxsz, ndtysz)  # Set initial text size
...
#
   ...
   call dr7220 (MOVE, home)      #, your mother misses you
...
#
   ...
   call dr7220 (MOVE, home)  # Home the plotter
...
#
   ...
   call dr7220 (MOVE, ptsin(1))                  # Move to first point
   call dp7220 (ndclrl)                          # Change color to line color
   j = 3
   for (i=2; i<=contrl(2); i=i+1) {              # Draw between subsequent pts
      call dr7220 (DRAW, ptsin(j))
      j = j + 2
      }
   }
...
#
   ...
   call dr7220 (MOVE, ptsin(1))  # Move to the first point
   ...
   for (i=1; i<=contrl(2); i=i+1) {       # Output marker at each point
      ...
      y1 = ycoord - hlfsiz
      y2 = y1 + fulsiz
      if ((min0(x1,y1) >= 0) & (x2 <= XxLIMITSxHP7220) & _
	 (y2 <= YxLIMITSxHP7220)) {           # Marker fits on device
 
	 call dp7220 (ndclrm)                 # Change color if necessary
 
	 call dt7220 (ndmkht, ndmkht)         # set marker size
         mrkout(3) = mrktbl(ndmktp)           # Get proper marker from table
         call dr7220 (MOVE, ptsin(j))         # Move to center point
         call dbufhp (4, mrkout)              # Enter marker mode
         call dr7220 (DRAW, ptsin(j))         # Display the marker
         mrkout(3) = SEMICOL
...
#
   ...
   call dr7220 (MOVE, ptsin(1))    # Move to start point

   call dp7220 (ndclrt)            # Change color to text color
...
#
   ...
   call dp7220 (ndclrf)                       # Change color to fill color
   ...
   call dr7220 (DRAW, ptsin(1))      # draw to first point to make sure
				     # polygon closed
   call dr7220 (MOVE, ptsin(1))      # Move to this point to pick up the point
...
#
   ...
   x2 = ptsin(3)
   y2 = ptsin(4)
   call dr7220 (MOVE, savexy)                 # Move to first point
   ...
   xcoord = savex
   call dr7220 (DRAW, xy)
   call dr7220 (DRAW, savexy)
   call dr7220 (MOVE, savexy)    # Move to that point to pick up the pen
...
#
   ...
   # cell excluding gap.  The 7220 plotter command for text size only requires
   ...
   call dt7220 (ndtxsz, ndtysz)              # Set the text size
...
#
   ...
   call dr7220 (MOVE, ptsin(1))        # move to initial point
...
#          ALL RIGHTS RESERVED.                                        #
#                                                                      #
#     Function: Device Driver move/draw for HP7220                     #
#                                                                      #
#     Input Parameters:                                                #
...
#                                                                      #
#     Routines Called:                                                 #
#            dbufhp - output a string to the HP7220 plotter            #
...
#          ALL RIGHTS RESERVED.                                        #
#                                                                      #
#     Function: Set text size on HP 7220 plotter                       #
#                                                                      #
#     Input Parameters:                                                #
...
#          ALL RIGHTS RESERVED.                                        #
#                                                                      #
#     Function: Change the color on the H P 7220 plotter               #
#                                                                      #
#     Input Parameters:                                                #
...
data penclr / BIGS, BIGP, DIG0, SEMICOL/  #  SP0;
   ...
   penclr(3) = color + DIG1  # Load this pen station
...
#          ALL RIGHTS RESERVED.                                        #
#                                                                      #
#     Function: HP7220 buffered output handler                         #
#                                                                      #
#     Input Parameters:                                                #
