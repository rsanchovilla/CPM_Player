...
specifivity it has is the fact that LOGO.COM file adheres CP/M 3.0 loader format
(see http://www.seasip.info/Cpm/rsxrec.html). 

This way, the operating system will to automatically load and start an RSX program 
embedded in LOGO.COM file. This RSX file is responsible of graphics/screen operations. 
...
- create a loader program that loads LGGSX on top of mem (this is LGLOAD)
- "GENGRAF" LDLOAD so it also loads defined GSX driver

To start generic GSX CP/M-80 2.2 logo, just run both programs in sequence
